...
/**
 * Sets up the animation timeline and camera behavior for a scrolling scene.
 *
 * The function creates a GSAP timeline that controls the camera's zoom, rotation, and position based on the progress of the scroll. It includes several helper functions to handle the different camera transformations at different stages of the scroll.
 *
 * The timeline is triggered by a ScrollTrigger that is attached to the '.sec1' and '.sec8' elements, allowing the animation to be scrubbed as the user scrolls.
 *
 * @param {Object} context - An object containing the camera, uniforms, and fboMaterial properties.
 * @returns {void}
 */
export const setupAnimation = (context) => {
  ...
  const zoomOut = (from = 0) => {
    ...
    camera.zoom =
      1.2 - process1 * 0.2 * (1 + 3 * THREE.MathUtils.clamp(progress2, 0, 0.4));
    camera.updateProjectionMatrix();
  };
  ...
  const rotateCamera2 = () => {
    ...
    camera.position.x =
      100 * Math.sin(Math.PI / 4 - 1.2 * 0.5 - progress * 0.6);
    camera.position.z =
      100 * Math.cos(Math.PI / 4 - 1.2 * 0.5 - progress * 0.6);
    camera.lookAt(0, 0, 0);
  };
  //   const zoomOut2 = () => {
